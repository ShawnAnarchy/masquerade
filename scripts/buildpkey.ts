...
function toMontgomeryQ(p:string) {
  const q = bigInt("21888242871839275222246405745257275088696311157297823662689037894645226208583");
  return bigInt(p).times(bigInt.one.shiftLeft(256)).mod(q);
}

function toMontgomeryR(p:string) {
  const r = bigInt("21888242871839275222246405745257275088548364400416034343698204186575808495617");
  return bigInt(p).times(bigInt.one.shiftLeft(256)).mod(r);
}
...
export type ProvingKey = {
  ...
  nPublic: number;
  domainSize: number;
  vk_alfa_1: Array<string>;
  vk_beta_1: Array<string>;
  vk_delta_1: Array<string>;
  ...
  B1: Array<Array<string>>;
  B2: Array<Array<string>>;
  C: Array<any>;
  hExps: Array<Array<string>>;
}

export function buildpkey(provingKey:ProvingKey){
